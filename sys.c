...
SYSCALL_DEFINE1(cs1550_down, struct cs1550_sem *, sem) {
	if (!sem || !sem->lock) {
		return -EINVAL; //validate semaphore and lock
	}

	mutex_lock(sem->lock); //acquire the mutex
	sem->value--;
	if (sem->value < 0){
		//allocate a new node
		...
		node->sleeping_task = current; //set the current process
		node->next = NULL;
        ...
        set_current_state(TASK_INTERRUPTIBLE); //set to interruptible sleep
        mutex_unlock(sem->lock); //release the mutex
        schedule(); //schedule other tasks
	} else {
		mutex_unlock(sem->lock); //release the mutex if not sleepinh
	}
	return 0;
}
...
SYSCALL_DEFINE1(cs1550_up, struct cs1550_sem *, sem) {
	...
	sem->value++;
    if (sem->value <= 0) {
        //wake up first process
        struct Node* temp = sem->head;
        wake_up_process(temp->sleeping_task); //wake up sleeping task
        sem->head = temp->next; //remove node from queue
        if (!sem->head) { //if queue is now empty, update tail
			sem->tail = NULL;
		}
        kfree(temp); //free
    }
...
SYSCALL_DEFINE2(cs1550_seminit, struct cs1550_sem *, sem, int, value) {
    if (!sem) {
        return -EINVAL; //return error if semaphore pointer is NULL

	}
    sem->value = value;
    sem->lock = (struct mutex *)kmalloc(sizeof(struct mutex), GFP_KERNEL);
    if (!sem->lock) {
        return -ENOMEM; //return error if memory allocation fails
	}
    mutex_init(sem->lock); //initialize the mutex
    sem->head = sem->tail = NULL; //wait queue as empty
    return 0; //success
}
